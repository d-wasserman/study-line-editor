...
def create_gap_filling_lines(input_line_features, output_feature_class, search_radius = "500 Feet", connection_count = 1):
    ...
    try:
        ...
        line_dict=  {pt_id: group for pt_id, group in groups}
        ...
        with arcpy.da.InsertCursor(output_feature_class, ["SHAPE@"]) as insert_cursor:
            for row in filtered_near_table:
                near_distance = row[2]
                if near_distance >= search_radius:
                    start_point = arcpy.da.SearchCursor(end_points_temp, ["SHAPE@"], "{0} = {1}".format(oid, row[0])).__next__()[0]
                    end_point = arcpy.da.SearchCursor(end_points_temp, ["SHAPE@"], "{0} = {1}".format(oid, row[1])).__next__()[0]
